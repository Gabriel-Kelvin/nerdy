...
begin
 ...
 if p_kind not in ('chat','report') then raise exception 'Invalid request'; end if;
 ...
 if r.used >= (case when p_kind='chat' then 100 else 10 end)
 or r.minute_used >= (case when p_kind='chat' then 10 else 2 end) then return false;end if;
