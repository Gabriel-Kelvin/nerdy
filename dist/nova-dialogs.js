...
export function mountNovaChat(modal,{avatar,request}){
 let history=[],active=true,pending=false,controller=null;
 ...
 async function send(text){
  ...
  pending=true;input.value='';input.disabled=true;submit.disabled=true;status.textContent='Nova is thinking…';
  ...
  finally{if(active){pending=false;input.disabled=false;submit.disabled=false;input.focus();}}
 }
 ...
 const dispose=()=>{if(!active)return;active=false;history=[];controller?.abort();modal.removeEventListener('close',onClose);modal.removeEventListener('cancel',dispose);modal.innerHTML='';};
