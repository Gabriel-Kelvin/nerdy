...
export function mountCarousel(root){
 ...
 let index=0,position=1,busy=false,timer,fallback,start;
 ...
 function schedule(){clearTimeout(timer);if(!reduced.matches&&!doc.hidden&&root.isConnected)timer=setTimeout(()=>{show(index+1);},6000);}
 function show(next){
 ...
 const motion=()=>{if(reduced.matches)settle();schedule();};reduced.addEventListener('change',motion);
 root.dataset.world='0';schedule();
